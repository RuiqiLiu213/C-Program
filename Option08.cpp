...
namespace fre {
	...
	double OptionCalculation::PriceByCRR(const BinomialTreeModel& Model, 
		BinLattice<double>& PriceTree, BinLattice<double>& XTree, BinLattice<double>& YTree)
	{
		double q = Model.RiskNeutProb();
		int N = pOption->GetN();
		PriceTree.SetN(N);
		...
		for (int n = N - 1; n >= 0; n--)
		{
			for (int i = 0; i <= n; i++)
			{
				...
				double Hu = PriceTree.GetNode(n + 1, i + 1);
				double Hd = PriceTree.GetNode(n + 1, i);
				//cout << Su<<"," << Sd << endl;
				ContPrice = (q * PriceTree.GetNode(n + 1, i + 1) + (1 - q) * PriceTree.GetNode(n + 1, i)) / Model.GetR();
				ContX = (Hu-Hd)/(Su-Sd);
				
				ContY =(Hd-ContX*Sd)/ pow(Model.GetR(),1+n);
				PriceTree.SetNode(n, i, ContPrice);
				XTree.SetNode(n, i, ContX);
				YTree.SetNode(n, i, ContY);
			}
		}
