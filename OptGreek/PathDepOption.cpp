...
namespace fre {
    ...
    {
        double H = 0.0, Hsq = 0.0, Heps = 0.0, Heps_ = 0.0;
        SamplePath S(m);
        for (long i = 0; i < N; i++)
        {
        ...
        gamma= exp(-Model.GetR() * T) * (Heps -2* H+ Heps_)/pow(Model.GetS0() * epsilon,2);
        return Price;
    }
    ...
    {
        double Ave = 0.0;
        for (int k = 0; k < m; k++) Ave = (k * Ave + S[k]) / (k + 1.0); //Ave is the average price
        if (Ave < K) return 0.0;
        return Ave - K;
    }
}
